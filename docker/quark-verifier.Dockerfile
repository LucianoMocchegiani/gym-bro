# Build Quark verifier para Compose GymBro.
...
WORKDIR /build/quark-verifier-service/source
COPY ssi-quark/quark-verifier-service/source/package*.json ./
COPY ssi-quark/quark-verifier-service/source/tsconfig.json ./
COPY ssi-quark/quark-verifier-service/source/src ./src
RUN npm ci \
  && npm install --no-save \
    @credo-ts/core@0.7.0 \
...
RUN npm ci --omit=dev \
  && npm install --omit=dev --no-save \
    ...
    @credo-ts/tenants@0.7.0 \
    express@^4.18.0
COPY --from=builder /build/quark-verifier-service/source/dist ./dist
EXPOSE 9002
