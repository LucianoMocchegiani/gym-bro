...
| RN-PAG-013 | **Débito automático** (post-MVP, diseño 2026-09-15): solo packs `MONTHLY`. Un mandato = afiliado + pack a debitar + **suscripción Mercado Pago** (`preapproval`) en la **cuenta MP del gym**. Un **plan** MP (`preapproval_plan`) por pack MONTHLY; si cambia el precio de catálogo, se actualiza el plan. No hay débito de drop-in ni `ONE_TIME`. GymBro **no** guarda PAN ni dispara el Payment recurrente (no Customer+Card+job). UI solo en Caja; la ficha redirige a `/caja?memberId=&vista=debitos`. Fuera: Stripe, CBU/DEBIN, alta desde la app. |
