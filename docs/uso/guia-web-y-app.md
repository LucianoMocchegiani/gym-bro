...
Todavía puede no tener pack: lo cobrás en Caja (§11) o él compra en la Tienda. El débito automático, si lo usás, se gestiona en Caja (la ficha tiene el atajo).
...
# Parte 3 — Módulos (para profundizar)

Misma lógica: qué hace, cómo se usa en web, **qué ve el socio**. Las capturas de primeros pasos se reusan; acá solo las que falten.

## Config

Ya cubierto en §05. Extra si hace falta:

**Captura:** `web-config-desconectado-mp.png`  
**Dónde:** MP sin conectar.  
**Para qué:** mostrar el estado “todavía no cobrás online”.

## Servicios

Ya cubierto en §06. Extra: servicio inactivo (no se vende, no aparece para armar pack nuevo).

**Captura:** `web-servicios-inactivo.png` — opcional.

## Packs

Ya cubierto en §07. Extra: pack mixto (libre + créditos) y qué pasa al cancelarlo (pierde todo).

**Captura:** `web-packs-lista.png` + `web-packs-componentes.png` — el mixto ya está.

## Sesiones

Ya cubierto en §08. Extra: roster (quién reservó) y lista de espera.

**Captura:** `web-sesiones-roster.png`  
**Dónde:** una sesión con 1–2 reservas.  
**Qué debe verse:** nombres (de prueba).

**Captura:** `app-mis-clases.png` — lista (reusa §08).

## Afiliados

Ya cubierto en §10. Extra: baja / no puede entrar.

**Captura:** `web-puerta-denegado-sin-pack.png` — ver Puerta.

## Staff y roles

Ya cubierto en §09. Extra: un rol sin Caja (el profesor no ve Caja) — ya está en `web-roles-flags.png`. Admin no se toca.
...
**Captura:** `web-reportes-movimientos.png` — lista (reusa §11).

## Puerta

En la **app**, el socio abre **Acceso**. Primero **acepta** la credencial (oferta del pack). Después, en puerta, presenta el QR / escanea según el flujo del local (no una captura de pantalla de un QR real de un cliente).
...
**Captura:** `web-afiliados-credencial.png` + `app-acceso-aceptar.png` — listas (reusa §10).
...
No documentar rutinas ni avisos hasta que estén en el corte de venta.

**Captura:** `app-ajustes.png`  
**Dónde:** Ajustes.  
**Qué debe verse:** cuenta / salir; ocultá URL de API si es pantalla de debug.
...
**Qué debe verse:** hilo + disclaimer.
