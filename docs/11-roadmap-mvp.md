...
- [x] Offer OID4VCI al pack APPROVED + `credential_offers` + `GET /me/credential-offers` (re-oferta = `POST /members/:id/credential-offers` del pack que cubre hoy; `packId` opcional)
...
- [x] Credential offers staff (afiliado OID4VCI en ficha)
  - Ficha afiliado: `GET /members/:id/credential-offers`; re-emitir `POST /members/:id/credential-offers` (`packId` opcional; pack que cubre hoy)
